...
az eventgrid event-subscription create --source-resource-id "/subscriptions/$SUBSCRIPTION_ID/resourceGroups/$RESOURCE_GROUP_NAME/providers/Microsoft.EventGrid/topics/$EVENT_GRID_TOPIC_NAME" --name $EVENT_GRID_SUB_NAME --endpoint-type 'webhook' --endpoint $EVENT_GRID_ENDPOINT --azure-active-directory-application-id-or-uri $SERVICE_PRINCIPAL_APP_ID
